    ...
    async def __make_request(
        ...
        try:
            async with make_request(
                ...
                if resp.status != 200:
                    # Only 401 means the session went stale; re-login once and retry.
                    # A 404 is a real "not found" (e.g. a disconnected foundation) and
                    # must NOT trigger a login loop -- surface it so callers can degrade.
                    if retry and resp.status == 401:
                        await self.login()
